...
__WaveNet__:
  Overall, the results for this model were both a success and failure. They were successful in the right that we were able to get them to run and produce some form of music, but not at the level of complexity we hoped for. The reason for this may be in the actual design of the model. Since WaveNet was designed for use with waveform data, and we had converted from the midi files to an integer sequence. The reason this could complicate things is in the training. Since the original wavenet had trained on each individual frequency at a given timestep of the data, sort of like a graph with nodess connecting them. We believed that we could sort of adapt that and thing of the song as a graph and the individual notes as the nodes. Also, something the creators had sort of bragged about was that they could throw anything in there, and get good results from it (allegedly). And perhaps this lack of structure was what had made the model work, due to the dropouts and dilations, which allows the model to generalize better. Nonetheless, the model did produce results, and here are some of them:
